 ...
 *
 * Generates MSW handlers for Phoenix API endpoints used by phoenix-insight.
 *
 * Why not use msw-auto-mock directly?
 * - Phoenix OpenAPI schema has circular references in OTLP types
 * - msw-auto-mock crashes on circular schema references
 * - We need more control over the mock data for testing
 *
 * This script generates handlers with realistic fixture data that:
 * - Returns success responses by default (happy path for testing)
 * - Can be customized with error handlers for error testing
 * - Includes realistic fake data using @faker-js/faker
 *
 * Usage:
 *   pnpm generate:mocks
 *
 * Or directly:
 *   npx tsx scripts/generate-msw-handlers.ts
 */
...
// Output path
const OUTPUT_PATH = join(projectRoot, "test/mocks/handlers.ts");
 ...
 *
 * These handlers mock the Phoenix API endpoints used by phoenix-insight:
...
writeFileSync(OUTPUT_PATH, handlersCode);

console.log("MSW Handler Generator");
console.log("=====================\n");
console.log(`Generated handlers for Phoenix API endpoints:\n`);
console.log("  - GET /v1/projects");
console.log("  - GET /v1/projects/:project_identifier/spans");
console.log("  - GET /v1/datasets");
console.log("  - GET /v1/datasets/:dataset_id/experiments");
console.log(`\nOutput: ${OUTPUT_PATH}`);
console.log("\nUsage in tests:");
console.log("  import { handlers, errorHandlers, fixtures } from './mocks/handlers';");
console.log("  import { setupServer } from 'msw/node';");
console.log("  const server = setupServer(...handlers);");
