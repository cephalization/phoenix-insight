...
## remove-msw-generator-script

### What Was Removed

1. **Deleted `scripts/generate-msw-handlers.ts`** - The script that supposedly "generated" MSW handlers from the Phoenix OpenAPI schema
2. **Removed `pnpm generate:mocks` script from `package.json`** - The npm script that invoked the generator

### Why This Was Necessary

The `generate-msw-handlers.ts` script was a false abstraction. It didn't actually generate handlers from the OpenAPI schema - `msw-auto-mock` crashes on Phoenix's circular OTLP type references. Instead, the script just wrote a hardcoded template string directly to `test/mocks/handlers.ts`. This is essentially a static file write disguised as "generation".

By removing the script:
- The handlers file is now recognized as what it is: a regular source file
- No fake "generation" step that doesn't actually generate anything
- Simpler maintenance - just edit `test/mocks/handlers.ts` directly
- One less script to understand/maintain

### What Was Checked

- **PROMPT.md Quick Reference table**: Does not mention `generate:mocks`, so no documentation update needed
- **All 384 tests pass**: Removing the script doesn't affect any functionality since the generated file already exists

### Follow-up Tasks

The next task (`remove-msw-auto-mock-dep`) will remove the `msw-auto-mock` devDependency since it's no longer needed. After that, `update-handlers-comments` will clean up the "Generated by" comments in `test/mocks/handlers.ts`.

## remove-msw-auto-mock-dep

### What Was Done

Removed `msw-auto-mock` from devDependencies using `pnpm remove msw-auto-mock`.

### Why This Was Necessary

The `msw-auto-mock` package was originally installed with the expectation that it could auto-generate MSW handlers from the Phoenix OpenAPI schema. However, as documented in `msw-generator-script` learnings, the Phoenix schema has circular references in OTLP types that cause `msw-auto-mock` to crash.

Since the generator script has been removed (see `remove-msw-generator-script`), the dependency is now unused. Removing it:
- Reduces devDependencies from 13 to 12 packages
- Eliminates the peer dependency warnings about zod version mismatches that appeared during install
- Removes 13 transitive dependencies that were only needed by msw-auto-mock

### Verification

- All 384 tests pass after removal
- No references to `msw-auto-mock` remain in the codebase
- The `msw` package (the actual Mock Service Worker) is retained and still works correctly

### Follow-up Task

The next task (`update-handlers-comments`) will clean up the "Generated by" comments in `test/mocks/handlers.ts` since the file is now a regular source file, not a generated one.

## update-handlers-comments

### What Was Changed

Updated the header comment in `test/mocks/handlers.ts` to:

1. **Removed "generated" references**: Replaced "Generated by: pnpm generate:mocks" and "Source: scripts/generate-msw-handlers.ts" with clear guidance that the file should be edited directly
2. **Added usage instructions**: Included a step-by-step guide for adding new handlers:
   - Add response type interface in "Types" section
   - Add fixture generator function in "Fixture Generators" section
   - Add pre-generated fixtures to `fixtures` object if needed
   - Add handler to `handlers` array
   - Optionally add error handlers to `errorHandlers` object
3. **Kept OpenAPI reference**: Preserved the `@see` link to the Phoenix OpenAPI schema as documentation for API structure
 ...
 *
 * Generated by: pnpm generate:mocks
 * Source: scripts/generate-msw-handlers.ts
 * ...
 */
```
...
The previous comments were misleading - they implied the file was auto-generated and should not be edited directly. This could cause confusion for developers who might:
- Look for the generator script (now deleted)
- Hesitate to make direct edits
- Try to regenerate the file

With the updated comments, it's clear that `test/mocks/handlers.ts` is a regular source file with clear instructions for extending it.
