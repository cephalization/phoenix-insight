...
## Phase 3: Remove MSW Generator Script

Goal: The `scripts/generate-msw-handlers.ts` script doesn't actually generate handlers from the OpenAPI schema - it just outputs a hardcoded template string. This was due to `msw-auto-mock` crashing on Phoenix's circular schema references. Since the "generation" is essentially writing a static file, remove the script and convert the handlers to a regular source file.

### remove-msw-generator-script

- content: Delete `scripts/generate-msw-handlers.ts` since it doesn't actually generate anything from the OpenAPI schema - it just writes a hardcoded template. The handlers in `test/mocks/handlers.ts` are already a complete, working source file. Remove the `pnpm generate:mocks` script from `package.json`. Update any documentation or comments that reference the generator script (check PROMPT.md Quick Reference table).
- status: complete
- dependencies: none
...
- content: Update the comments in `test/mocks/handlers.ts` to remove references to being "generated" (e.g., "Generated by: pnpm generate:mocks"). The file is now a regular source file that should be edited directly. Keep the OpenAPI schema reference link as documentation for the API structure. Ensure the file has clear comments explaining its purpose and how to add new handlers.
- status: pending
- dependencies: remove-msw-auto-mock-dep
